...
当前版本支持环境巡检、本地安装包安装/升级、更新检查、隔离下载、配置备份/恢复、OpenClaw 安全升级试点、安装位置策略预览和安装命令预演，并已接入 Google Chrome 官方企业版离线 MSI 和 Git for Windows 离线安装包的检测与安装包预演。

v2.5.3 在 v2.5.2 新电脑实测稳定性修正版基础上新增 Git 支持。Git 可用于新电脑首次离线安装，安装后会刷新当前会话 PATH 并复检 `git --version`，但默认不自动升级。
...
- `Claude Desktop` 已补充 `HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\AnthropicClaude`、`%LOCALAPPDATA%\AnthropicClaude`、`%LOCALAPPDATA%\AnthropicClaude\claude.exe` 等检测规则。
- `Google Chrome` 使用 `chrome.msi`，按官方企业版离线 MSI 安装包处理；浏览器涉及登录、书签、扩展、Cookie、配置和自动更新机制，默认不自动升级。
- `Git` 使用 `git.exe`，按 Git for Windows 离线安装包处理；安装会影响 PATH、Git Bash、右键菜单和凭据管理，允许新电脑首次离线安装，但默认不自动升级。
- `ToDesk` 是远控软件，静默安装参数不可靠，当前默认人工确认，不参与 `[1]` 自动静默安装。
...
- 高风险或中高风险软件如微信、QQ、ToDesk、Clash、智谱AI输入法、Google Chrome 默认不自动下载、不自动升级。
- Git 是开发环境基础组件，可用于新电脑首次离线安装；已安装时默认跳过，不覆盖安装、不自动升级。
- Codex 桌面端 `codex.exe` 是 Store Installer 在线引导器，可能需要联网和 Microsoft Store 组件，不保证离线安装成功。
- Google Chrome 当前使用官方企业版离线 MSI，可用于新电脑首次安装；已安装时默认跳过，不覆盖安装、不自动升级、不强制指定安装目录。
- ToDesk 默认不自动静默安装，需人工确认后手动安装。
...
7. 再选择 `[11] 安装命令预演`，确认每个安装包和预演命令。
8. 如 C 盘空间紧张，继续选择 `[10] 安装位置策略预览`。
9. 确认报告无误后再考虑使用 `[1]` 或其他会修改系统的功能。
